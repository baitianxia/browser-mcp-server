...
普通用户双击一次即可进入完整流程，工具对其全部 Claude Code 项目生效，且不会污染仓库或因 UAC 切换身份而把 MCP 注册到错误账号。浏览器拒绝包内 CRX 策略安装，或当前用户无权读写企业锁定的策略键时，用户需在扩展页加载一次屏幕给出的本地目录；自动打开页面和复制目录失败时仍显示完整指引，原安装进程等待并继续，不要求重跑。代价是每个需要使用该 MCP 的 Windows 用户都要各自安装一次，并且 pilot 对被批准 Tab 中的现有登录身份具有操作能力。
