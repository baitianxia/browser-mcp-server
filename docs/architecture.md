...
上图是生产 persistent 基线。Windows 通用 pilot 初始使用 `Claude Code → stdio → Playwright MCP → Playwright Extension → 用户批准的现有浏览器会话`；安装后可选择保存当前用户扩展授权，或切换到不读取日常登录态的独立 Profile。
