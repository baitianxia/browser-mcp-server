...
- 已有受管 Windows pilot 再次运行顶层 launcher 时属于升级。安装器必须在运行时发布和旧配置移动前验证旧清单身份与全部受管路径，只提取 ADR-0009/0010 定义的用户选择，并用新版固定模板、已验证运行时和重新识别的浏览器路径生成配置；不得复制旧运行路径或未知字段。仍可用的原浏览器 channel 应在自动模式下保留。旧版没有 `interaction` 时只为新增交互选项采用 `compact + robust`。当前用户扩展令牌只能从现有 Claude user-scope 条目读取并通过会被注册器消费的短期环境传递，不得进入暂存偏好或日志；独立 Profile 升级必须跳过扩展安装。来源不受管、组合不合法或令牌无法安全保留时，在移动旧配置前失败关闭。
