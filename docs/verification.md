...
该阶段工作树已经把用户身份收敛为 `browser-mcp-server` / “浏览器助手” / `browser-mcp`，公共包收敛为 `browser-mcp-server-<版本>-windows-x64.zip`，安装与配置根目录收敛为 `%USERPROFILE%\browser-mcp-server`。历史身份、旧 MCP 别名和旧目录迁移明确不属于本次 breaking release。
...
1.0.13 把更新清单、策略键读/建/写、策略写后核对和自动启动浏览器统一限制在可选策略尝试内。未发生策略变更时，任一失败都记录 `OFFLINE EXTENSION POLICY UNAVAILABLE` 并直接显示扩展页地址与完整目录，在原安装进程等待人工加载；自动打开扩展页和 `clip.exe` 失败也只是警告。策略值已经实际写入时仍必须恢复原值，只有恢复失败才因状态不安全停止。为避免继续逐项暴露同类问题，运行时、扩展、配置备份/发布和配置回滚现共用 `[IO.Directory]::Move` 原子移动与有界重试；旧版留下的无效扩展目录会先保留到唯一备份路径再自动重建，失败配置也隔离而不递归删除。

本地已执行 90 项完整测试：88 项通过，2 项仅因需要 Windows PowerShell 5.1 而跳过；GitHub Actions YAML、全部 Python AST、shell 语法和 `git diff --check` 通过。[Windows release validation #54](https://github.com/baitianxia/intranet-browser-agent/actions/runs/33376504639) 已在提交 `c1982ed246e15e1a9de7ec45b1c4a2a031e11b1c` 上全绿：`windows-2022`、`windows-latest` 两套源码/PowerShell 5.1 job 与 Windows 原生 package job 全部成功。下载的发布者门禁日志实际运行 90 项测试并输出 `OK (skipped=2)`、`CLAUDE MCP REGISTRATION SELF-TEST PASSED`、包内 Node/MCP smoke 和 `WINDOWS POWERSHELL 5.1 RELEASE GATE PASSED`。

package job 使用与目标日志一致的系统 Node v20.18.3 和真实 npm Claude Code 2.1.84，安装器自动选择包内 Node v24.19.0，目标流程没有运行 npm、pnpm、npx 或 Claude 安装/修复命令。job 先预置无效扩展目录并验证自动隔离重建，再分别施加真实运行时目录 ACL 拒绝和 Chrome 策略 `SetValue` 拒绝；同一个顶层 launcher、同一个安装器进程依次记录 `RUNTIME PUBLISH RETRY 1/25`、`RUNTIME PUBLISH RECOVERED: attempts=2`、`OFFLINE EXTENSION POLICY UNAVAILABLE`、`MANUAL EXTENSION LOAD REQUIRED`、`MANUAL EXTENSION LOAD DETECTED` 与最终 `SUCCESS`。原本不存在的临时策略键及全部父键按逆序恢复为不存在。安装日志只有一次启动记录且不含 `self-test`、`unittest` 或发布测试名称。随后实际安装的 Node + Playwright MCP 对离线页面完成 `navigate`、`snapshot` 和 Cookie 登录态复用，输出 `PLAYWRIGHT EXTENSION MCP E2E PASSED: server=1.63.0-alpha-2026-08-05, tools=30, session_cookie=reused`。

run #54 的 GitHub 迁移 artifact ZIP SHA-256 为 `9afbcb2dce6a69b6095b9a0a09ce2a1028e569b9437a27840177eb16170a40fb`，证据 artifact ZIP 为 `bacc9949261a8b7066bdab02f25ff571cb39f913ab25f0eee498fabc02633408`；两者下载后逐字节复算一致且 ZIP 结构通过。迁移 archive SHA-256 为 `6528c82109309304a7bcc22ebb68e73056fa934b5529e522dd19f66f68f3d7ac`，包内 Windows 原生 runtime archive 为 `f0f3e0739e96686ea95b3cf7e0e2318b6ecc4fb39f2a652f336df94ab420f789`；迁移 archive、解压迁移目录、runtime archive、解压 runtime 目录四层均由制品内当前校验器返回 `VALID`。元数据记录 69 个白名单文件，`buildHost=windows/x64`、`target=windows/x64`、`crossBuilt=false`、`targetCliSmokeTested=true`、`bundledNode=true`。为交给用户一次解压，已把同一验证通过的迁移目录无内容改写地封装为单顶层目录 ZIP；其 SHA-256 为 `88fbfe5b5cae198842f264cd6c912b67f1afb15b91cb2644488909548330c1b8`，无 `__MACOSX`/`.DS_Store`，解压后再次由包内校验器返回 `VALID`。

[Windows release validation #50](https://github.com/baitianxia/intranet-browser-agent/actions/runs/33372817902) 的 `windows-2022`、`windows-latest` 两套源码/AST job 已通过；package job 在启动目标 launcher 前由新增故障注入夹具失败，未生成 artifact。GitHub Runner 原本没有 Chrome 策略父键，PowerShell Registry provider 的单次 `New-Item -Force` 不会递归创建完整父链，随后 `Get-Acl` 得到路径不存在。[#51](https://github.com/baitianxia/intranet-browser-agent/actions/runs/33373444807) 改用 .NET `RegistryKey.CreateSubKey` 后两套源码 job 仍通过，但 package job 紧接着通过 PowerShell Registry provider 的 `Get-Acl` 读取时仍报路径不存在。[#52](https://github.com/baitianxia/intranet-browser-agent/actions/runs/33374871244) 又以同一 Provider 逐段创建、逐段 `Test-Path` 成功，却仍在随后的 `Get-Acl` 得到同一错误；两套源码 job 依然通过，package job 仍在 launcher 前停止。这组证据把问题收敛为 GitHub Runner 上注册表 `Get-Acl/Set-Acl` 夹具不可靠，而不是键创建方式。[#53](https://github.com/baitianxia/intranet-browser-agent/actions/runs/33375887295) 改用原生注册表安全描述符后首次证明拒绝真实生效，并让同一 launcher 进入安装器：现有 npm Claude、包内 Node、离线扩展、无效扩展遗留隔离、运行时目录访问拒绝后的自动重试/恢复均通过，策略写入被拒绝后也正确显示人工加载目录并保持原进程等待。该 run 最终只因夹具对原本不存在、随后本应整体删除的临时策略键提前比较 ACL 字符串而停止；Windows 规范化继承 ACE 后字符串不同不表示原状态未恢复，因此没有 artifact。#54 修正夹具后完成全链路放行。四个失败 run 均未冒充安装通过或生成可交付制品。
...
- 源码测试：当前本地构建环境执行 `PYTHONDONTWRITEBYTECODE=1 python3 -m unittest discover -s tests -v`，90 项中 88 项通过、2 项仅因需要 Windows PowerShell 5.1 而跳过。新增覆盖明确断言策略访问、浏览器自动启动和剪贴板失败进入同一进程人工回退，扩展与配置所有目录切换共用原子重试且安装器不含 `Move-Item`；其余覆盖包括目标 launcher/installer 不调用发布门禁、`unittest` 或注册器 `self-test`，顶层 launcher 只产生一个安装器进程，以及 Windows 路径、user scope、版本一致性、LF/CRLF 逐字节备份、规范 LF archive sidecar、`remove/add/get` 注册失败回滚、条目/环境错写、窄 Windows 代码页与 Claude UTF-8 输出组合、遗留环境清理、Node 来源、archive 路径、扩展 CRX/Profile/人工等待、休眠 Profile 防误判、显式 browser `.exe`、配置漂移、发布顺序、发布访问拒绝恢复和 MCP stdio 握手。run #54 的 Windows PowerShell 5.1 发布者门禁实际运行同一组 90 项测试，88 项通过、2 项按迁移包边界跳过。
...
- 运行包：run #54 已由 GitHub Actions 在 Windows x64 上使用固定 Node v24.19.0、pnpm 11.19.0 原生重建 `browser-agent-runtime-1.0.13-core-windows-x64.tar.gz`；制品记录 `buildHost=windows/x64`、`target=windows/x64`、`crossBuilt=false`、`targetCliSmokeTested=true`、`bundledNode=true`，并以包内 `node.exe` 完成版本与 MCP 握手后才组装迁移包。任何本地交叉构建结果仍必须记录 `crossBuilt=true`、`targetCliSmokeTested=false`，不得冒充原生制品。
...
- 安装事务：运行时先在同盘短目录解压，完整性、Windows 原生发布元数据、Node 批准哈希和实际版本通过后才发布固定版本目录。发布遇到短暂访问拒绝时，仅在源仍存在且目标不存在的安全状态下由同一个安装进程有界重试，成功后重新验证最终目录；run #54 已用真实 ACL 拒绝证明首次失败、第二次恢复且安装器只启动一次。配置先在 `%LOCALAPPDATA%` 暂存并 preflight，再整目录切换；“旧目录备份完成”和“新目录发布完成”独立记账，备份动作本身失败时不会删除原目录。后续失败恢复旧配置和 Claude 用户配置。
- Windows 门禁：发布方在 Windows CI 中单独运行 `verify-windows-release.ps1`，完成单元测试、注册器 `self-test`、原生 Claude 隔离注册和 MCP 握手后才允许组装可下载制品。目标顶层 `.cmd` 规范化自身解压目录后只启动一次 `INSTALL-WINDOWS-PILOT.ps1`；不运行发布门禁、`unittest`、注册器 `self-test`、npm、pnpm 或 npx。run #54 的 package job 先用原生 Claude Code 2.1.84 完成独立发布门禁，再从 `PATH` 隐藏原生入口、暴露真实 npm `claude.cmd`，证明单次目标安装选择现有 npm 安装并以目标机同版本 Node v20.18.3/CLI 完成注册；CI 的 Internet 出站阻断只用于证明离线能力，不会由目标安装器修改用户防火墙。安装器从运行时访问拒绝自动恢复，在 Chrome 策略写入被拒绝时进入人工扩展等待并从同一进程自动续跑，最后由实际安装 MCP 完成离线页面与 Cookie 登录态复用 E2E。
- 迁移包：1.0.13 共包含 69 个逐文件白名单内的源码/文档文件；仍不收录 `.git`、真实部署清单、`build/`、`dist/`、缓存或 Python bytecode。run #54 的迁移 archive、解压迁移目录、包内 runtime archive、解压 runtime 目录四层均返回 `VALID`；发布测试与注册器 `self-test` 的实现仍随包保留以便审计，但目标 launcher/installer 不调用它们。迁移 archive SHA-256 `6528c82109309304a7bcc22ebb68e73056fa934b5529e522dd19f66f68f3d7ac` 与 LF sidecar 和验证证据三方一致，标准 `shasum -a 256 -c` 已实际通过。
