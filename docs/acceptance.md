...
- 同一个 Windows package job 必须在已安装状态下再次调用顶层 `INSTALL-WINDOWS-PILOT.cmd`：先证明 `extension + 当前用户令牌 + full + standard` 覆盖升级后逐项不变、令牌没有进入安装日志；再证明 `persistent + 独立 Profile + headless + compact + robust` 覆盖升级后逐项不变，且安装日志明确跳过扩展安装和人工扩展等待。源码测试还必须覆盖缺少 `interaction` 的旧版清单只为新增选项采用默认值，以及非受管路径、错误类型和未知选项在旧配置移动前被拒绝。两次升级日志必须进入验证证据，任一步失败不得上传正式包。
