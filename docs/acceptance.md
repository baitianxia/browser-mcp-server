...
- 每次推送到 `main` 的 `.github/workflows/windows-release.yml` 必须在 `windows-2022`、`windows-latest` 两个 Windows x64 runner 上以 Windows PowerShell 5.1 跑完源码测试和全部 PowerShell AST 解析；Windows 原生打包 job 还必须使用批准的 Node v24.19.0 来源、固定 pnpm 11.19.0 和原生 Claude Code 2.1.84，从解压迁移包执行顶层 `INSTALL-WINDOWS-PILOT.cmd`。该 job 必须阻断 Chrome 出站以证明目标安装不依赖网络；若进入人工加载回退，由 CI 专用 remote-debugging-pipe 探针通过目录输入枚举安装器显示的同一目录，把文件按原相对路径和原始字节写入 `chrome://extensions` 的临时 FileSystem，再把其根 `DirectoryEntry` 交给 Chrome 的持久 `developerPrivate.loadDirectory` Profile importer。GitHub 托管 runner 没有可代表真人的受信任桌面输入，因此不得声称 CI 点过原生目录选择框；也不得以 `Extensions.loadUnpacked` 的当前会话结果代替持久安装，因为该状态可能在 Chrome 重启后被清除。导入路径只能是 last-used Profile 下固定的 `Unpacked Extensions/<批准目录名>`，且探针和安装检测器都必须逐文件核对其类型、大小和 SHA-256 与批准目录一致；随后还必须核对扩展已启用、ID/版本正确，在 Chrome 退出后仍可由安装器检测，并证明原 launcher 自动续跑。安装完成后还必须在 Chrome 仍被阻断出站时，由实际安装的 `node.exe + @playwright/mcp` 通过该扩展调用 `browser_navigate` 和 `browser_snapshot` 访问本机离线页面，并证明页面收到探针预置到同一 Profile 的会话 Cookie；一次性 CI 令牌不得写入迁移包、安装配置、日志或证据。最终同时保留发布门禁 PASS、安装 SUCCESS、安装摘要、user-scope 配置和安装后运行时完整性证据。失败 run 的产物不得作为 Windows 已验证迁移包交付。
...
- Windows pilot 必须固定 `extension` 模式、显式绑定自动识别的 Chrome/Edge channel 和实际浏览器 `.exe`、不得配置 `userDataDir` 或扩展 token，并保留人工连接批准。安装检测只能接受浏览器 last-used Profile，不能因其他休眠 Profile 中存在扩展而误放行。缺少扩展时先尝试包内 CRX 的当前用户离线策略安装；只有浏览器检测到扩展才能继续。策略安装未生效时必须恢复临时策略、自动打开扩展页、显示/复制包内已解压目录并在同一个安装进程中等待；人工加载成功后自动续跑，不得要求第二次启动。检测器可以接受人工加载的精确批准目录，或 Chrome 持久 importer 在 last-used Profile 固定 `Unpacked Extensions/<批准目录名>` 生成的逐字节一致副本；后者必须拒绝 link/junction/reparse point、路径偏移、缺失/增加文件及任意内容变化。自动回归必须覆盖 CRX/已解压目录篡改拒绝、扩展 Profile 检测、Profile import 逐文件核对、休眠 Profile 防误判、人工回退步骤顺序、等待后续跑及临时策略恢复。
