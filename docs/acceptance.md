...
- 每次推送到 `main` 的 `.github/workflows/windows-release.yml` 必须在 `windows-2022`、`windows-latest` 两个 Windows x64 runner 上以 Windows PowerShell 5.1 跑完源码测试和全部 PowerShell AST 解析；Windows 原生打包 job 还必须使用批准的 Node v24.19.0 来源、固定 pnpm 11.19.0 和原生 Claude Code 2.1.84，从解压迁移包执行顶层 `INSTALL-WINDOWS-PILOT.cmd`。该 job 必须阻断 Chrome 出站以证明目标安装不依赖网络；若进入人工加载回退，由 CI 专用 remote-debugging-pipe 探针在 `chrome://extensions` 创建 `webkitdirectory` 输入，通过 `DOM.setFileInputFiles` 选择安装器显示的同一目录，并把浏览器生成的 `DirectoryEntry` 交给 Chrome 的持久 `developerPrivate.loadDirectory` 实现。GitHub 托管 runner 没有可代表真人的受信任桌面输入，因此不得声称 CI 点过原生目录选择框；也不得以 `Extensions.loadUnpacked` 的当前会话结果代替持久安装，因为该状态可能在 Chrome 重启后被清除。探针必须核对扩展已启用、ID/版本和目录正确，并在 Chrome 退出后仍可由安装器检测，随后证明原 launcher 自动续跑。安装完成后还必须在 Chrome 仍被阻断出站时，由实际安装的 `node.exe + @playwright/mcp` 通过该扩展调用 `browser_navigate` 和 `browser_snapshot` 访问本机离线页面，并证明页面收到探针预置到同一 Profile 的会话 Cookie；一次性 CI 令牌不得写入迁移包、安装配置、日志或证据。最终同时保留发布门禁 PASS、安装 SUCCESS、安装摘要、user-scope 配置和安装后运行时完整性证据。失败 run 的产物不得作为 Windows 已验证迁移包交付。
