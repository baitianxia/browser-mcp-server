...
- 运行时必须解压到同盘唯一暂存目录并在发布最终版本目录前验证；安装器必须取得当前用户独占安装锁。暂存目录到版本目录的原子移动必须把访问拒绝、共享冲突和等价目录移动 I/O 错误视为可恢复的短暂占用，在同一个安装进程中按有界退避重试；重试期间不得重跑发布门禁，成功后必须再次验证最终目录。自动 Windows 验收必须在完整解压后保存暂存目录原 ACL、对当前用户真实且可逆地拒绝该目录的 `Delete` 权限，使首次发布移动产生操作系统访问拒绝；观察到安装日志的首次重试后必须逐字恢复原 ACL，并证明顶层 launcher 只运行一次、安装日志同时包含重试与恢复标记、最终安装和 E2E 仍成功。每次重试必须确认源仍存在且目标不存在；目标已出现、状态不明确或恢复窗口耗尽时必须失败关闭。配置必须在 `%LOCALAPPDATA%` 内暂存、preflight、整目录切换，状态明确时后续失败恢复旧目录；检测到外部并发导致状态不明确时必须保留当前目录和备份并失败关闭，不得删除无法确定所有权的目录。写入前必须拒绝越出 `%LOCALAPPDATA%` 或经过其子级 link/junction 的目标路径；已有版本不得覆盖。
