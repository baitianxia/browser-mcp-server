...
class WindowsPilotSetupTests(unittest.TestCase):
    ...
    def test_windows_ci_uses_persistent_ui_extension_and_session_e2e(self) -> None:
        ...
        self.assertIn("RunspaceFactory", selector)
        self.assertIn("ApartmentState]::STA", selector)
        self.assertIn("BeginInvoke()", selector)
        self.assertIn("action=sta-runspace-invoke", selector)
        self.assertIn("UIA_ASYNC_LOAD action=returned", selector)
        self.assertIn("AsyncWaitHandle.WaitOne(20000)", selector)
